...
parser = ArgumentParser(description="OPLS for FA saturation.")
    ...
    for polarity in polarities:
        ...
        for i,condition in zip(range(3),conditions):
            for j,pop in zip(range(1,4),pops):
                category_subset = sum_by_category.loc[:,sum_by_category.columns.str.contains(conditions[condition]) & sum_by_category.columns.str.contains(pop)]
                classes_subset = sum_by_class.loc[:,sum_by_class.columns.str.contains(conditions[condition]) & sum_by_class.columns.str.contains(pop)]
...
for tissue in tissues:
    for condition in conditions:
        ...
        for pop in pops:
            ...
            df = df.loc[(df['Category'] == 'Saturated Fatty Acids') | (df['Category'] == 'Monounsaturated Fatty Acids') | (df['Category'] == 'Polyunsaturated Fatty Acids')]#)
        ...
        for comp,groups in comparisons.items():
            ...
            df = df.apply(log10)
            #https://stackoverflow.com/questions/22799208/keep-finite-entries-only-in-pandas
            ...
            pathname = os.path.join(args.output_dir,f'opls/{outlier_text}/{tissue}/{condition}/fas')
