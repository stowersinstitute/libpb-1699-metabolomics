...
outlier_str = 'no-outliers' if args.exclude_outlier else 'outliers'
    ...
    for tissue in tissues:
        for cond in condmap:
            for comp in comparisons:
                d = read_csv(f'out/work/primary/glm/singlefactor/{outlier_str}/{cat}/{tissue}/{cond}/{comp}.csv',index_col=0)
                d['Category'] = cat
                d['Tissue'] = tissue
                d['Condition'] = cond
                d['Comparison'] = comp
...
catplot('Condition', 'Value', data=data, kind='point', row='Compound',hue='Population',col='Tissue',sharey=False,palette=['firebrick','goldenrod','dodgerblue'],capsize=0.1,height=4.)
